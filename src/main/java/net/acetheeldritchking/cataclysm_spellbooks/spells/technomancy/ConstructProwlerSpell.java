...
public class ConstructProwlerSpell extends AbstractHarbingerSpell {
    private final ResourceLocation spellId = ResourceLocation.fromNamespaceAndPath(CataclysmSpellbooks.MOD_ID, "construct_prowler");
    ...
    public void onCast(Level level, int spellLevel, LivingEntity entity, CastSource castSource, MagicData playerMagicData) {
        ...
        for (int i = 0; i < spellLevel; i++)
        {
            ...
            spawnProwler(randomNearbyX, vec.y, randomNearbyZ, entity, level, summonTimer, castData);
        }
    ...
    private void spawnProwler(double x, double y, double z, LivingEntity caster, Level level, int summonTimer, SummonedEntitiesCastData castData)
    {
        ...
        SummonManager.initSummon(caster, prowler, summonTimer, castData);
    }
}
